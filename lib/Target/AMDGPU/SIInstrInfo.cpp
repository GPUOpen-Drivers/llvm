...
void SIInstrInfo::legalizeOperandsVOP2(MachineRegisterInfo &MRI,
  ...
  // both the value to write (src0) and lane select (src1), and the value to
  // use for other lanes (src2) must be VGPR. Fix up non-SGPR src0/src1 with
  // V_READFIRSTLANE.
  if (Opc == AMDGPU::V_WRITELANE_B32) {
    ...
    if (Src2.isReg() && RI.isSGPRReg(MRI, Src2.getReg()))
...
void SIInstrInfo::moveToVALU(MachineInstr &TopInst) const {
  ...
  while (!Worklist.empty()) {
    ...
    case AMDGPU::S_BUFFER_LOAD_DWORDX16_SGPR: {
      ...
      legalizeOperands(*NewInstr, Worklist);
      continue;
    }
    }
    ...
    if (NewOpcode == AMDGPU::INSTRUCTION_LIST_END) {
      ...
      legalizeOperands(Inst, Worklist);
      continue;
    }
...
void SIInstrInfo::splitScalar64BitAddSub(
  ...
  // valid.
  legalizeOperands(*LoHalf, Worklist);
  legalizeOperands(*HiHalf, Worklist);
