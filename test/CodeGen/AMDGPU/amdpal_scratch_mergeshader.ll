...
; On gfx9 and later, a HS is a merge shader, in which s0-s7 are reserved by the
; hardware, so the PAL puts the GIT (global information table) in s8 rather
; than s0.

; CHECK-LABEL: {{^}}_amdgpu_hs_main:
; CHECK: s_getpc_b64 s{{\[}}[[GITPTR:[0-9]+]]:
...
attributes #4 = { "amdgpu-max-work-group-size"="128" }
